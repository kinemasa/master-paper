...
# --- プログラムで subject_id と roi_name を設定 ---
# ここを自由に変更できます（例: 自動抽出や固定値）
# 例①: 固定値を設定
...
for new_subject_id in num_subject_ids:
    ...
    for key, old_path in data["paths"].items():
        ...
        # ROI部分を置換 (_glabella_before や _chin_after など)
        new_path = re.sub(r"_[A-Za-z0-9]+_(before|after)", f"_{new_roi_name}", old_path)
        
        # subject_id部分を置換（ファイル名だけ変更）
